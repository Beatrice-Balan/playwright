...
test.describe("API e2e test together with DB", () => {
  ...
  test('Update a student we created using PUT', async ({ request, updatedStudent }) => {
        // Step 1: Send PUT without headers (to match Test 1 & 2)
        ...
        // const responseBody = await response.json();
        // compareResponseWithRequest(responseBody, updatedStudent)
  })
...
/**
   ...
   */
    ...
    test('Delete the student using DELETE', async({ request, updatedStudent }) => {
        ...
        // Step 2: Validate status is 2xx
        expect(response.ok()).toBeTruthy()

        // Step 3: Check DB to confirm deletion
        const query = `SELECT * FROM students WHERE email = ${updatedStudent.EMAIL}`
        const result = await executeQuery(query)

        // Validate no student is found
